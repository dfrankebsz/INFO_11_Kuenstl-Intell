...
const STORAGE = {
  token:'neuroquest-session',
  guest:'neuroquest-guest-progress-v1',
  cache:'neuroquest-cloud-cache-v1',
  guestStarted:'neuroquest-guest-started'
};
...
function achievements(){
  ...
  const arr=[
    ['🔓','Blackbox geöffnet',c.length>=1],
    ['📥','Input-Profi',sectionDone(sectionById.inout)],
    ['🧩','Hidden-Scout',sectionDone(sectionById.hidden)],
    ['🎚️','Gewichte-Profi',sectionDone(sectionById.weights)],
    ['⚡','Aktivierungs-Profi',sectionDone(sectionById.activation)],
    ['🕸️','Netz-Versteher',sectionDone(sectionById.fullnet)],
    ['🚀','KI-Designer/in',sectionDone(sectionById.final)],
    ['🏆','NeuroQuest gemeistert',coreCompleted()===CORE_TASK_IDS.length]
  ];
...
function openSection(id,scroll=true){
  ...
  $('#breadcrumb').textContent=`Neuronale Netze · ${sectionById[id].nav}`;
...
function bindInteractiveLabs(){
  const weight=$('#weightLab');
  if(weight){
    const inputs=[1,0.7,0.5];
    const sliders=$$('input[data-w]',weight);
    const update=()=>{
      const ws=sliders.map(s=>Number(s.value));
      sliders.forEach(s=>s.nextElementSibling.textContent=Number(s.value).toFixed(1));
      const sum=inputs.reduce((acc,x,i)=>acc+x*ws[i],0);
      $('#weightSum').textContent=sum.toFixed(2);
      $('#weightDecision').textContent=`Ausgabe: ${sum>=1?1:0}`;
    };
    sliders.forEach(s=>s.addEventListener('input',update));update();
  }
  const act=$('#activationLab');
  if(act){
    const sl=$('#activationInput');
    const update=()=>{
      const z=Number(sl.value), sig=1/(1+Math.exp(-z));
      $('#activationInputOut').textContent=z.toFixed(1);
      $('#thresholdOut').textContent=z>=0?'1':'0';
      $('#reluOut').textContent=Math.max(0,z).toFixed(1);
      $('#sigmoidOut').textContent=sig.toFixed(2);
    };
...
function renderAll(){
  ...
  renderNav();renderSection();updateHeader();
  $('#breadcrumb').textContent=`Neuronale Netze · ${sectionById[state.activeSection].nav}`;
}
